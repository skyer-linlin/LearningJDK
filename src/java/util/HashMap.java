...
/**
 * Hash table based implementation of the {@code Map} interface. This implementation provides all of
 * the optional map operations, and permits {@code null} values and the {@code null} key. (The
 * {@code HashMap} class is roughly equivalent to {@code Hashtable}, except that it is
 ...
 *
 * <p>This implementation provides constant-time performance for the basic operations ({@code get}
 * and {@code put}), assuming the hash function disperses the elements properly among the buckets.
 * Iteration over collection views requires time proportional to the "capacity" of the {@code
 * HashMap} instance (the number of buckets) plus its size (the number of key-value mappings). Thus,
 * it's very important not to set the initial capacity too high (or the load factor too low) if
 * iteration performance is important.
 *
 * <p>An instance of {@code HashMap} has two parameters that affect its performance: <i>initial
 ...
 * of the operations of the {@code HashMap} class, including {@code get} and {@code put}). The
 ...
 *
 * <p>If many mappings are to be stored in a {@code HashMap} instance, creating it with a
 ...
 * through the iterator's own {@code remove} method, the iterator will throw a {@link
...
public class HashMap<K, V> extends AbstractMap<K, V> implements Map<K, V>, Cloneable, Serializable {
   ...
   *
   * <p>哈希数组,首次使用时初始化,必要时扩容.分配空间时,数组长度始终是2的幂. 当前无需引导机制时,某些操作中也可以接受数组长度为0.
   */
   ...
   * <p>扩容阈值 = 容量 * 负载因子
   *
   * @serial The javadoc description is true upon serialization. Additionally, if the table array
  ...
  /**
   * Constructs an empty {@code HashMap} with the default initial capacity (16) and the default load
   * factor (0.75).
   */
   ...
   *
   * <p>在map中将特定的键和值关联起来。如果旧key已经存在，则旧值将被替换
   *
   ...
   *     <p>与{@code key}关联的旧值，如果key之前不存在，则返回{@code null}. 如果返回值为{@code
   *     null},也可能是之前key对应的value为{@code null}.
   */
   ...
   * {@inheritDoc}
   *
   * <p>This method will, on a best-effort basis, throw a {@link ConcurrentModificationException} if
  ...
  /**
   * Returns a shallow copy of this {@code HashMap} instance: the keys and values themselves are not
   * cloned.
   *
   ...
   *
   * <p>计算key.hashCode（）并将（xor）高位散列扩展到低位.
   */
  /*
  ...
  static final int hash(Object key) {
    int h;
    // note 将高16位数据移到低16位再按位异或
    // note 因为有些数据计算出的哈希值差异主要在高位，而 HashMap 里的哈希寻址是忽略容量以上的高位的，那么这种处理就可以有效避免类似情况下的哈希碰撞
  ...
  static final int tableSizeFor(int cap) {
    /*
     * note
     * >> 表示向右移位，如果该数为正，则高位补0，如果该数为负数，则高位补1
     * >>> 表示逻辑右移，无论正数或者负数，向右移位后高位统一补0
     */
    // todo 学习这里的取最相近的2的幂数的实现
   ...
   * @param evict if false, the table is in creation mode. 若为false,表示为创建模式(creation mode)
  ...
  final V putVal(int hash, K key, V value, boolean onlyIfAbsent, boolean evict) {
    ...
    // n为数组长度,按hash计算落在数组哪个桶(bucket)中
    // note n的值为2^n-1,二进制表示为111111...1,则按位与结果为hash的低位,整个计算等价于hash%n
    p = tab[i = (n - 1) & hash];
   ...
   * table.
   *
   * <p>初始化或者扩容数组.如果数组为空,以类中持有的初始化容量字段进行分配.否则,因为使用的是2的幂拓展,bin(链表)的每个元素在数组拓展的时候都得移动到新的空间上
   *
   * @return the table
   */
  /*
  ...
  final Node<K, V>[] resize() {
    ...
    if (oldCap > 0) {
      ...
      if (oldCap >= MAXIMUM_CAPACITY) {
        // 将HashMap的阈值更新为允许的最大值(在无法扩容的情况下,阈值存在已经没有意义了)
      ...
      // 如果实例化HashMap时已经指定了初始容量，则将哈希数组当前容量初始化为与旧阈值一样大（初始容量与旧阈值的计算关系参见tableSizeFor()方法,向上取2的幂）
      if (oldThr > 0) {
        newCap = oldThr;
        ...
        if (e != null) {
          ...
          if (e.next == null) {
            ...
            do {
              ...
              if ((e.hash & oldCap) == 0) { // 所在的桶不变
                // 如果是桶中的第一个元素,则loHead=e,loTail=e
                if (loTail == null) {
                  loHead = e;
                } else {
                  // 如果不是桶中第一个元素,更新当前最后一个元素的next,更新loTail的指针指向e
                  loTail.next = e;
                }
                loTail = e;
              } else { // 所在的桶发生改变,如原来共16个桶,链表中的元素在第3个桶中,则现在在第19(=16+3)桶中
                if (hiTail == null) {
                  hiHead = e;
                } else {
                  hiTail.next = e;
            ...
            // 检查桶中是否有元素,如果有元素,更新链表中的链接关系,防止数组复制前的元素链接关系还在
            if (loTail != null) {
              loTail.next = null;
              newTab[j] = loHead;
            }
  ...
  /**
   * Entry for Tree bins. Extends LinkedHashMap.Entry (which in turn extends Node) so can be used as
          ...
          for (TreeNode<K, V> p = root; ; ) {
            ...
            if ((ph = p.hash) > h) {
              ...
              // 待插入结点的hash值出现了雷同
                    ...
                    && (kc = comparableClassFor(k)) == null) // 如果k（的类类型）没有实现“有效”的Comparable接口

 ...
 * most one value.
 *
 * <p>This interface takes the place of the {@code Dictionary} class, which was a totally abstract
 * class rather than an interface.
 *
 * <p>The {@code Map} interface provides three <i>collection views</i>, which allow a map's contents
 ...
 * views return their elements. Some map implementations, like the {@code TreeMap} class, make
 * specific guarantees as to their order; others, like the {@code HashMap} class, do not.
 *
 * <p>Note: great care must be exercised if mutable objects are used as map keys. The behavior of a
 * map is not specified if the value of an object is changed in a manner that affects {@code equals}
 ...
 * contain itself as a value, extreme caution is advised: the {@code equals} and {@code hashCode}
 * methods are no longer well defined on such a map.
 *
 * <p>All general-purpose map implementation classes should provide two "standard" constructors: a
 * void (no arguments) constructor which creates an empty map, and a constructor with a single
 * argument of type {@code Map}, which creates a new map with the same key-value mappings as its
 * argument. In effect, the latter constructor allows the user to copy any map, producing an
 * equivalent map of the desired class. There is no way to enforce this recommendation (as
 * interfaces cannot contain constructors) but all of the general-purpose map implementations in the
 * JDK comply.
 *
 * <p>The "destructive" methods contained in this interface, that is, the methods that modify the
 * map on which they operate, are specified to throw {@code UnsupportedOperationException} if this
 * map does not support the operation. If this is the case, these methods may, but are not required
 * to, throw an {@code UnsupportedOperationException} if the invocation would have no effect on the
 ...
 *
 * <p>Some map implementations have restrictions on the keys and values they may contain. For
 * example, some implementations prohibit null keys and values, and some have restrictions on the
 * types of their keys. Attempting to insert an ineligible key or value throws an unchecked
 * exception, typically {@code NullPointerException} or {@code ClassCastException}. Attempting to
 * query the presence of an ineligible key or value may throw an exception, or it may simply return
 * false; some implementations will exhibit the former behavior and some will exhibit the latter.
 * More generally, attempting an operation on an ineligible key or value whose completion would not
 ...
 *
 * <p>Many methods in Collections Framework interfaces are defined in terms of the {@link
 * Object#equals(Object) equals} method. For example, the specification for the {@link
 * #containsKey(Object) containsKey(Object key)} method says: "returns {@code true} if and only if
 * this map contains a mapping for a key {@code k} such that {@code (key==null ? k==null :
 * key.equals(k))}." This specification should <i>not</i> be construed to imply that invoking {@code
 * Map.containsKey} with a non-null argument {@code key} will cause {@code key.equals(k)} to be
 * invoked for any key {@code k}. Implementations are free to implement optimizations whereby the
 * {@code equals} invocation is avoided, for example, by first comparing the hash codes of the two
 * keys. (The {@link Object#hashCode()} specification guarantees that two objects with unequal hash
 * codes cannot be equal.) More generally, implementations of the various Collections Framework
 ...
 *
 * <p>Some map operations which perform recursive traversal of the map may fail with an exception
 * for self-referential instances where the map directly or indirectly contains itself. This
 * includes the {@code clone()}, {@code equals()}, {@code hashCode()} and {@code toString()}
 * methods. Implementations may optionally handle the self-referential scenario, however most
 * current implementations do not do so.
 *
 * <h2><a id="unmodifiable">Unmodifiable Maps</a></h2>
 *
 * <p>The {@link Map#of() Map.of}, {@link Map#ofEntries(Map.Entry...) Map.ofEntries}, and {@link
 * Map#copyOf Map.copyOf} static factory methods provide a convenient way to create unmodifiable
 * maps. The {@code Map} instances created by these methods have the following characteristics:
 *
 * <ul>
 *   <li>They are <a href="Collection.html#unmodifiable"><i>unmodifiable</i></a>. Keys and values
 *       cannot be added, removed, or updated. Calling any mutator method on the Map will always
 *       cause {@code UnsupportedOperationException} to be thrown. However, if the contained keys or
 *       values are themselves mutable, this may cause the Map to behave inconsistently or its
 ...
 *   <li>They are <a href="../lang/doc-files/ValueBased.html">value-based</a>. Callers should make
 *       no assumptions about the identity of the returned instances. Factories are free to create
 *       new instances or reuse existing ones. Therefore, identity-sensitive operations on these
 *       instances (reference equality ({@code ==}), identity hash code, and synchronization) are
 *       unreliable and should be avoided.
 *   <li>They are serialized as specified on the <a
 *       href="{@docRoot}/serialized-form.html#java.util.CollSer">Serialized Form</a> page.
 * </ul>
 *
...
public interface Map<K, V> {
   ...
   *     associated {@code null} with {@code key}, if the implementation supports {@code null}
   *     values.)
   ...
   * Objects.equals(key, k)}, that mapping is removed. (The map can contain at most one such
   * mapping.)
   *
   ...
   *     this method and document its concurrency properties.
   * @param key key with which the specified value is associated
   * @param newValue newValue to be associated with the specified key
   * @return the previous value associated with the specified key, or {@code null} if there was no
   *     mapping for the key. (A {@code null} return can also indicate that the map previously
   *     associated {@code null} with the key, if the implementation supports null values.)
   * @throws UnsupportedOperationException if the {@code put} operation is not supported by this map
   ...
   * such that {@code Objects.equals(value, v)}. This operation will probably require time linear in
   * the map size for most implementations of the {@code Map} interface.
   *
   ...
   * @param bakValue the non-null value to be merged with the existing value associated with the key
  ...
  default V merge(
      K key, V bakValue, BiFunction<? super V, ? super V, ? extends V> remappingFunction) {
    Objects.requireNonNull(remappingFunction);
    Objects.requireNonNull(bakValue);

    V oldValue = get(key);

    V newValue = (oldValue == null) ? bakValue : remappingFunction.apply(oldValue, bakValue);
   ...
   * function itself throws an (unchecked) exception, the exception is rethrown, and no mapping is
   * recorded. The most common usage is to construct a new object serving as an initial mapped value
   * or memoized result, as in:
   *
   * <pre>{@code
   ...
   *     modifies this map during computation and, if appropriate, reporting an error.
   *     Non-concurrent implementations should override this method and, on a best-effort basis,
   *     throw a {@code ConcurrentModificationException} if it is detected that the mapping function
   *     modifies this map during computation. Concurrent implementations should override this
   *     method and, on a best-effort basis, throw an {@code IllegalStateException} if it is
   *     detected that the mapping function modifies this map during computation and as a result
   *     computation would never complete.
   *     <p>The default implementation makes no guarantees about synchronization or atomicity
   *     properties of this method. Any implementation providing atomicity guarantees must override
   *     this method and document its concurrency properties. In particular, all implementations of
   *     subinterface {@link java.util.concurrent.ConcurrentMap} must document whether the mapping
   *     function is applied once atomically only if the value is not present.
   * @since 1.8
   */
  ...
  /**
   * Returns an unmodifiable {@link Entry} containing the given key and value. These entries are
   * suitable for populating {@code Map} instances using the {@link Map#ofEntries Map.ofEntries()}
   * method. The {@code Entry} instances created by this method have the following characteristics:
   *
   * <ul>
   *   <li>They disallow {@code null} keys and values. Attempts to create them using a {@code null}
   *       key or value result in {@code NullPointerException}.
   *   <li>They are unmodifiable. Calls to {@link Entry#setValue Entry.setValue()} on a returned
   *       {@code Entry} result in {@code UnsupportedOperationException}.
   *   <li>They are not serializable.
   *   <li>They are <a href="../lang/doc-files/ValueBased.html">value-based</a>. Callers should make
   *       no assumptions about the identity of the returned instances. This method is free to
   *       create new instances or reuse existing ones. Therefore, identity-sensitive operations on
   *       these instances (reference equality ({@code ==}), identity hash code, and
   ...
   * @apiNote For a serializable {@code Entry}, see {@link AbstractMap.SimpleEntry} or {@link
   ...
   * @apiNote It is convenient to create the map entries using the {@link Map#entry Map.entry()}
   *     method. For example,
   *     <pre>{@code
   ...
   * @implNote If the given Map is an <a href="#unmodifiable">unmodifiable Map</a>, calling copyOf
   *     will generally not create a copy.
   * @since 10
   */
  // 返回对指定映射的浅拷贝
  ...
  static <K, V> Map<K, V> copyOf(Map<? extends K, ? extends V> map) {
    if (map instanceof ImmutableCollections.AbstractImmutableMap) {
      ...
      // 将指定的键值对包装到只读容器
      return (Map<K, V>) Map.ofEntries(entries);
    }
  }
  ...
  /**
   * A map entry (key-value pair). The {@code Map.entrySet} method returns a collection-view of the
   * map, whose elements are of this class. The <i>only</i> way to obtain a reference to a map entry
   * is from the iterator of this collection-view. These {@code Map.Entry} objects are valid
   ...
   * except through the {@code setValue} operation on the map entry.
   *
   * @see Map#entrySet()
   * @since 1.2
   */
  ...
  interface Entry<K, V> {
     ...
     * already been removed from the map (by the iterator's {@code remove} operation).
     *
     * @param value new value to be stored in this entry
     * @return old value corresponding to the entry
     * @throws UnsupportedOperationException if the {@code put} operation is not supported by the
     *     backing map
     ...
     * any two Entries {@code e1} and {@code e2}, as required by the general contract of {@code
     * Object.hashCode}.
     *
